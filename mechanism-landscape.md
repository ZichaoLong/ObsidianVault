...
> [!summary] 本页定位
> 本页整理递归分解与 memory 线需要面对的完整对标谱系。它回答三个问题：第一，当前 D² / Phase 1 到底落在什么机制谱系里；第二，未来 `verified subproblem memory` 要对标哪些强路线；第三，HRM / TRM / RLM 这类递归模型应如何在本线中定位。
...
- RLM 比 HRM 更直接相关，因为 RLM 把长上下文外部化为环境，并允许模型 inspect / decompose / recursively call itself；它是递归 self-call 与外部上下文管理的强基线。
- HRM / TRM 是内部隐状态递归路线，不能作为显式 memory 或 verified subproblem memory 的证据。
...
- 它把上下文外部化为可操作环境。
- 它允许模型递归调用自身处理局部片段。
- 它很接近“递归 self-call + 外部上下文环境”的接口型递归路线。
...
HRM / TRM 与本线的显式子问题对象路线不同：

- 没有显式 subproblem graph。
- 没有可检索、可复用、可审计的外部 memory item。
- 没有 proof / test / verifier 绑定的中间 artifact。
- 递归发生在模型内部隐状态，而不是外显的任务结构中。

它们对本线的意义是：

> 递归计算不一定要外显为 Agent trace 或 memory item；也可以内化为 recurrent hidden state。本线若要主张 `verified subproblem memory` 有价值，必须证明外显中间对象在可验证性、复用性、审计性、迁移性或工程可控性上提供额外收益。

RLM 对本线的压力更直接：

- 它把长 prompt 当外部环境。
- 模型可以 inspect / decompose / recurse。
- 如果本线只做递归 self-call 或长上下文局部处理，RLM 是强基线。
- 本线必须进一步证明 verified memory、typed subproblem graph 或可复用 artifact 的增量。
...
- HRM/TRM/RLM 只作为递归计算谱系中的相邻路线，用来界定本线的边界和强基线。
