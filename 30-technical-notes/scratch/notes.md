...
### [**Shannon熵**](https://en.wikipedia.org/wiki/Entropy_(information_theory))

$H(\pmb{X})$可用于描述随机变量$\pmb{X}$的不确定性，表示消息[随机变量$\pmb{X}$的一次观测值]带来的**信息量**[[自信息](https://en.wikipedia.org/wiki/Information_content)$-\log_b(P(\pmb{X}))$，$P(\pmb{X})$越小代表信息量越大]的期望值，也可理解为针对概率分布$P(\pmb{X})$进行编码[参考[Huffman coding](https://en.wikipedia.org/wiki/Huffman_coding)、[Arithmetic coding](https://en.wikipedia.org/wiki/Arithmetic_coding)]的**编码长度期望值**下限[参考[AI & 无损压缩科普博客](https://zhuanlan.zhihu.com/p/651212186)]
$$
...
### [**相对熵, KL散度(Kullback-Leibler divergence)**](https://en.wikipedia.org/wiki/Kullback–Leibler_divergence)
...
D_{KL}(P\|Q)\geq0,
$$
注：非负性[证明1]由$\ln x\leq x-1$可得$\sum P\ln(Q/P)\leq\sum P(Q/P-1)=0$。[证明2]由于$\log z$关于$z\in\mathbb{R}^+$是上凸函数，记$z_X=Q(X)/P(X)$，由[Jensen不等式](https://en.wikipedia.org/wiki/Jensen's_inequality)，不严格的推导可得$-D_{KL}(P\|Q)=\sum_XP(X)\log z_X\leq\log(\sum_XP(X)\cdot z_X)=\log\sum_XQ(X)=0$。
...
[**Pinsker不等式(Pinsker's Inequality)**](https://en.wikipedia.org/wiki/Pinsker%27s_inequality) 指出[全变差距离](https://en.wikipedia.org/wiki/Total_variation_distance_of_probability_measures)具有由KL散度[[#^eq-relative-entropy|公式]]表示的上界。对$X$上的两个概率测度$P,Q$，其全变差距离定义为
$$
\begin{aligned}
\delta(P,Q)
  &=\frac12\int_X|P(x)-Q(x)|\,\mathrm{d}x \\
  &=\int_{P(x)>Q(x)}[P(x)-Q(x)]\,\mathrm{d}x \\
  &=\int_{P(x)<Q(x)}[Q(x)-P(x)]\,\mathrm{d}x.
\end{aligned}
$$
它与KL散度($b=e$，熵单位nats)满足不等式
$$
\delta(P,Q)\leq \sqrt{\frac12D_{KL}(P\|Q)}
\tag{3}
$$
^eq-pinsker-inequality
...
$$
注：[[#^eq-pinsker-inequality|Pinsker 不等式]]可证明如下（By [John Pollard](https://en.wikipedia.org/wiki/John_Pollard_(mathematician)），记$r(x)=\frac{P(x)}{Q(x)}-1\geq-1$，则$D_{KL}(P\|Q)=E_Q[(1+r(x))\ln(1+r(x))-r(x)]\geq\frac12E_Q\frac{r^2(x)}{1+r(x)/3}\geq\frac12\frac{E_Q^2(|r(x)|)}{E_Q(1+r(x)/3)}=2\delta^2(P,Q)$。

### [交叉熵(Cross entropy)](https://en.wikipedia.org/wiki/Cross-entropy)

$H(P,Q)$表示**基于一个“错误”概率分布**$Q$[相对于真实概率分布$P$]进行编码的编码位长度的期望值，
$$
\begin{split}
...
继[[#Shannon熵|定义了熵]]之后，还可两个随机变量$\pmb{X},\pmb{Y}$的条件熵，也就是**假设已知**$\pmb{X}$**时**$\pmb{Y}$**的新增信息量**，定义为
$$
\begin{split}
...
等式$H(\pmb{X},\pmb{Y})=H(\pmb{X})+H(\pmb{Y}|\pmb{X})$的**直观解读**为：联合随机变量$(\pmb{X},\pmb{Y})$的信息量$H(\pmb{X},\pmb{Y})$，也就是联合熵，等于$\pmb{X}$的信息量$H(\pmb{X})$+假设已知$\pmb{X}$时$\pmb{Y}$的新增信息量$H(\pmb{Y}|\pmb{X})$。
...
两个随机变量$\pmb{X},\pmb{Y}$的互信息，也就是它们**共享的信息量**，定义为
$$
\begin{aligned}
I(\pmb{X};\pmb{Y})
  &= H(\pmb{X})+H(\pmb{Y})-H(\pmb{X},\pmb{Y}) \\
  &= H(\pmb{X},\pmb{Y})-H(\pmb{X}|\pmb{Y})-H(\pmb{Y}|\pmb{X}) \\
  &= H(\pmb{X})-H(\pmb{X}|\pmb{Y})
     =H(\pmb{Y})-H(\pmb{Y}|\pmb{X}) \\
  &= D_{KL}\big(P(\pmb{X},\pmb{Y})\|P(\pmb{X})\cdot P(\pmb{Y})\big) \\
  &= E_{\pmb{X},\pmb{Y}}\left(
      \log\frac{P(\pmb{X},\pmb{Y})}
      {P(\pmb{X})\cdot P(\pmb{Y})}\right)
\end{aligned}
\tag{6}
$$
^eq-mutual-information
当$\pmb{X},\pmb{Y}$相互独立即$P(\pmb{X},\pmb{Y})=P(\pmb{X})\cdot P(\pmb{Y})$时，其互信息达到最小值0。等式$H(\pmb{Y})=I(\pmb{X};\pmb{Y})+H(\pmb{Y}|\pmb{X})$的**直观解读**为：$\pmb{Y}$的信息量$H(\pmb{Y})$，等于$\pmb{X},\pmb{Y}$的共享信息量$I(\pmb{X};\pmb{Y})$+假设已知$\pmb{X}$时$\pmb{Y}$的新增信息量$H(\pmb{Y}|\pmb{X})$。

条件互信息$I(\pmb{X};\pmb{Y}|\pmb{Z})$，也就是**假设已知**$\pmb{Z}$**时**$\pmb{X},\pmb{Y}$**共享的信息量**，定义为
$$
\begin{aligned}
I(\pmb{X};\pmb{Y}|\pmb{Z})
  &=H(\pmb{X}|\pmb{Z})+H(\pmb{Y}|\pmb{Z})
    -H(\pmb{X},\pmb{Y}|\pmb{Z}) \\
  &=H(\pmb{X}|\pmb{Z})-H(\pmb{X}|\pmb{Y},\pmb{Z}) \\
  &=\mathbb{E}_{\pmb{z}\sim P(\pmb{Z})}
    \left[I(\pmb{X};\pmb{Y}|\pmb{Z}=\pmb{z})\right]
\end{aligned}
\tag{7}
$$
^eq-conditional-mutual-information

等式$H(\pmb{X}|\pmb{Z})=H(\pmb{X}|\pmb{Y},\pmb{Z})+I(\pmb{X};\pmb{Y}|\pmb{Z})$的**直观解读**为：已知$\pmb{Z}$时$\pmb{X}$的新增信息量，等于已知$\pmb{Y},\pmb{Z}$时$\pmb{X}$的新增信息量+已知$\pmb{Z}$时$\pmb{X},\pmb{Y}$的共享信息量。

条件互信息满足如下的链式分解，有两种表述
$$
\begin{aligned}
I(\pmb{X};\pmb{Y},\pmb{Z})
  &=I(\pmb{X};\pmb{Z})+I(\pmb{X};\pmb{Y}|\pmb{Z}) \\
  &=I(\pmb{X};\pmb{Y})+I(\pmb{X};\pmb{Z}|\pmb{Y}) \\
I(\pmb{X};\pmb{Y}|\pmb{Z})
  &=I(\pmb{X};\pmb{Y})-I(\pmb{X};\pmb{Z})
    +I(\pmb{X};\pmb{Z}|\pmb{Y}) \\
  &=I(\pmb{X};\pmb{Y})-I(\pmb{Y};\pmb{Z})
    +I(\pmb{Y};\pmb{Z}|\pmb{X})
\end{aligned}
\tag{8}
$$
^eq-chain-rule-for-mutual-information
只需使用条件熵与互信息之间的等式关系即可证明上述链式分解，其**直观解读**（以第一个表述的第一个等号为例）为：$\pmb{X}$与联合随机变量$\pmb{Y},\pmb{Z}$的共享信息量，等于$\pmb{X}$与$\pmb{Z}$的共享信息量+已知$\pmb{Z}$时$\pmb{X}$与$\pmb{Y}$的共享信息量。

对于多个变量，可推广至定义[交互信息](https://en.wikipedia.org/wiki/Interaction_information)，及条件联合互信息
$$
\begin{aligned}
I(\pmb{X}_1;\cdots;\pmb{X}_{n+1})
  &= I(\pmb{X}_1;\cdots;\pmb{X}_{n})
     -I(\pmb{X}_1;\cdots;\pmb{X}_{n}|\pmb{X}_{n+1}) \\
I(\pmb{X}_1;\cdots;\pmb{X}_n|\pmb{X}_{n+1})
  &= \sum_{i=1}^nH(\pmb{X}_i|\pmb{X}_{n+1})
     -H(\pmb{X}_1,\cdots,\pmb{X}_n|\pmb{X}_{n+1}) \\
  &= D_{KL}\bigg(
      P\big((\pmb{X}_1;\cdots;\pmb{X}_{n})|\pmb{X}_{n+1}\big)
      \,\big\|\,P(\pmb{X}_1|\pmb{X}_{n+1})
      \otimes\cdots\otimes P(\pmb{X}_n|\pmb{X}_{n+1})
     \bigg)
\end{aligned}
$$
...
$$
H(\pmb{X},\pmb{Y})\quad v.s.\quad A\cup B
$$
...
I(\pmb{X};\pmb{Y})
  &=H(\pmb{X})+H(\pmb{Y})-H(\pmb{X},\pmb{Y}) \\
  &\quad v.s.\quad A\cap B=A+B-A\cup B \\
I(\pmb{X};\pmb{Y}|\pmb{Z})
  &=H(\pmb{X}|\pmb{Z})+H(\pmb{Y}|\pmb{Z})
    -H(\pmb{X},\pmb{Y}|\pmb{Z}) \\
  &\quad v.s.\quad (A\cap B)/C=A/C+B/C-(A\cup B)/C
\end{aligned}
$$

与Venn图的代数结构略有区别的是，信息论中通常不讨论补运算。但基于上述对应关系，仍然可以大大帮助我们理解和推导信息论中的基本运算，基于上述对应关系中的公式，可以得到[[#^eq-mutual-information|互信息公式]]、[[#^eq-conditional-mutual-information|条件互信息公式]]和[[#^eq-chain-rule-for-mutual-information|链式分解公式]]中的剩下所有运算关系，并对应其**直观解读**。

需注意，两个随机变量的互信息$I(\pmb{X};\pmb{Y})$或关于第三个变量的条件互信息$I(\pmb{X};\pmb{Y}|\pmb{Z})$，按照定义$I(\pmb{X};\pmb{Y})=H(\pmb{X})+H(\pmb{Y})-H(\pmb{X},\pmb{Y})$、$H(\pmb{X}|\pmb{Z})+H(\pmb{Y}|\pmb{Z})-H(\pmb{X},\pmb{Y}|\pmb{Z})$可解读为Venn图，但对于多个随机变量时的交互信息不再能如此解读。事实上，互信息、条件联合互信息、交互信息与Venn图的联系，是通过熵、联合熵、条件熵来建立，转化之后即可使用Venn图的运算法则和理解。
...
3. 三角不等式：$\sqrt{D_{JS}(P\|Q)}\leq\sqrt{D_{JS}(P\|R)}+\sqrt{D_{JS}(R\|Q)}$。
   注：可先证明$L:\mathbb{R}^+\times\mathbb{R}^+\to\mathbb{R}^+, L(p,q)=p\ln\frac{2p}{p+q}+q\ln\frac{2q}{p+q}$满足$\sqrt{L(p,q)}\leq\sqrt{L(p,r)}+\sqrt{L(r,q)}$，再结合Minkowski不等式，即可证明该性质。参考[Endres, Dominik Maria, and Johannes E. Schindelin. "A new metric for probability distributions." *IEEE Transactions on Information theory* 49.7 (2003): 1858-1860.](https://research-repository.st-andrews.ac.uk/bitstream/handle/10023/1591/Endres2003-IEEETransInfTheory49-NewMetric.pdf;jsessionid=7EA22DA7E50A8672DC247B1AAEFC62F2?sequence=1)
...
注：JS散度可作为全变分度量的下界，若其对数基底使用$b=2$，则$D_{JS}(P\|Q)\leq\|P-Q\|_{l_1}\leq1$，一般的有$D_{JS}(P_1,\cdots,P_n)\leq\log_2n$，对于其他基底，将不等式右边改成$\log_bn$即可。
...
D_F(\pmb{p},\pmb{q})
  &= F(\pmb{p})-F(\pmb{q})
     -\left\langle\nabla_{\pmb{q}}F(\pmb{q}),\pmb{p}-\pmb{q}\right\rangle \\
  &\approx (\pmb{p}-\pmb{q})^T\cdot\nabla^2_{\pmb{q}}F(\pmb{q})
     \cdot(\pmb{p}-\pmb{q}),
     \qquad \pmb{p}\approx\pmb{q}
\end{aligned}
$$
则$D_F(\pmb{p},\pmb{q})\geq0$，且由$F$的严格凸性，等号成立当且仅当$\pmb{p}=\pmb{q}$。

1. 使得$D_F$满足度量的对称性的唯一可能是$F$是正定二次型，这时$\sqrt{D_F}$就是仿射变换下的欧氏度量。
2. 取$F$为负[[#Shannon熵|熵]]函数$F(\pmb{p})=\sum_ip_i\ln p_i$，此时Bregman散度$D_F$就是广义的[[#相对熵, KL散度(Kullback-Leibler divergence)|KL散度]]，$D_F(\pmb{p},\pmb{q})=\sum_{i}p_i\ln\frac{p_i}{q_i}-\sum_ip_i+\sum_iq_i$，当$\pmb{p},\pmb{q}$表示离散概率，于是$\sum_ip_i=\sum_iq_i=1$时，$D_F$就是通常意义的KL散度。
...
\begin{aligned}
\mathcal{I}_X(\theta)
  &= E\left[\left(\frac{\partial\mathcal{L}}{\partial\theta}\right)^2
      \middle|\theta\right] \\
  &= -E\left[\frac{\partial^2\mathcal{L}}{\partial\theta^2}
      \middle|\theta\right]
\end{aligned}
$$
...
注：上述定义中第2项与第3项的恒等性，可基于$E\left[\frac{\partial\ln P(X;\theta)}{\partial\theta}|\theta\right]=\partial_\theta \int P\mathrm{d}X=\partial_\theta1=0$，以及$E\left[\frac{\partial_\theta^2P(X;\theta)}{P(X;\theta)}|\theta\right]=\partial_\theta^2\int P\mathrm{d}X=\partial_\theta^21=0$来化简证明。
...
1. 它表示了无偏估计$\theta$的精度下界，即[Cramér–Rao不等式(CRLB)](https://en.wikipedia.org/wiki/Cramér–Rao_bound)，任何一个无偏估计公式/方法$\hat{\theta}(X)$，其估计方差满足
   ...
   Cramér–Rao不等式的证明如下。由无偏条件，
   $$
   \int (\hat{\theta}-\theta)P(X;\theta)\,\mathrm{d}X=0.
   $$
   对参数$\theta$求导，得到
   $$
   \begin{aligned}
   0
     &=\frac{\partial}{\partial\theta}
       \int (\hat{\theta}-\theta)P\,\mathrm{d}X \\
     &=-\int P\,\mathrm{d}X
       +\int (\hat{\theta}-\theta)\frac{\partial P}{\partial\theta}
        \,\mathrm{d}X.
   \end{aligned}
   $$
   因此
   ...
   \,\mathrm{d}X=1.
   $$
   进一步，由[Cauchy–Schwarz不等式](https://en.wikipedia.org/wiki/Cauchy–Schwarz_inequality)，
   $$
   \begin{aligned}
   Var(\hat{\theta}(X))\,\mathcal{I}_X(\theta)
     &=\left[\int(\hat{\theta}(X)-\theta)^2P(X;\theta)\,\mathrm{d}X\right]
       \left[\int\left(\frac{\partial_\theta P}{P}\right)^2
       P(X;\theta)\,\mathrm{d}X\right] \\
     &\geq\left[\int(\hat{\theta}-\theta)\frac{\partial P}{\partial\theta}
       \,\mathrm{d}X\right]^2=1.
   \end{aligned}
   $$
   也就是$Var(\hat{\theta}(X))\geq\frac1{\mathcal{I}_X(\theta)}$。

2. 给定$X$观测值区分$\theta$与$\theta+\delta\theta$，其敏感度可通过两个分布$P(X;\theta),P(X;\theta+\delta\theta)$之间的距离来度量，
   $$
   D_{KL}\big(P(X;\theta)\|P(X;\theta+\delta\theta)\big)
     \approx\frac12\delta\theta^2\cdot I_X(\theta)
   $$
   注：对上述KL散度沿$\theta$维度进行Taylor展开，可得
   $$
   \begin{aligned}
   D_{KL}
     &=-\int P(X;\theta)\ln
       \frac{P(X;\theta+\delta\theta)}{P(X;\theta)}\,\mathrm{d}X \\
     &\approx-\delta\theta\int P\,\partial_\theta\mathcal{L}\,\mathrm{d}X
       -\frac12\delta\theta^2\int P\,\partial_\theta^2\mathcal{L}\,\mathrm{d}X \\
     &=-\delta\theta\cdot(\cdots)
       +\frac12\delta\theta^2\cdot I_X(\theta).
   \end{aligned}
   $$
   又因为
   $$
   \int P(X;\theta)\partial_\theta\mathcal{L}(X;\theta)\,\mathrm{d}X
     =\int\partial_\theta P(X;\theta)\,\mathrm{d}X
     =\partial_\theta\int P\,\mathrm{d}X
     =\partial_\theta1=0,
   $$
   一阶项为$0$，所以
   $$
...
$$
\begin{aligned}
\mathcal{I}_{X,ij}(\pmb{\theta})
  &= E\left[
      \big(\partial_{\theta_i}\ln P(X;\pmb{\theta})\big)
      \big(\partial_{\theta_j}\ln P(X;\pmb{\theta})\big)
      \middle|\pmb{\theta}\right] \\
  &= -E\left[
      \partial_{\theta_i\theta_j}\ln P(X;\pmb{\theta})
      \middle|\pmb{\theta}\right]
\end{aligned}
$$
...
$$
\begin{aligned}
D_{KL}\big(P(X;\pmb{\theta})\|P(X;\pmb{\theta}+\delta\pmb{\theta})\big)
  &\approx \frac12\delta\pmb{\theta}^T
     \cdot\pmb{\mathcal{I}}\cdot\delta\pmb{\theta}
\end{aligned}
$$
...
其基本直觉是，数据处理之后，不会新增信息，这一直觉的具体数学表述是数据处理不等式。考虑三个随机变量，$\pmb{X},\pmb{Y},\pmb{Z}$，其中$\pmb{X},\pmb{Z}$是关于给定$\pmb{Y}$条件独立的，即$P(\pmb{Z}|\pmb{Y})=P(\pmb{Z}|\pmb{X},\pmb{Y}),P(\pmb{X}|\pmb{Y})=P(\pmb{X}|\pmb{Z},\pmb{Y}),P(\pmb{X},\pmb{Y},\pmb{Z})=P(\pmb{X}|\pmb{Y})P(\pmb{Z}|\pmb{Y})P(\pmb{Y})$，记作$(\pmb{X}\perp\!\!\!\perp\pmb{Z}|\pmb{Y})$，则无法从$\pmb{Z}$中获得比$\pmb{Y}$更多的关于$\pmb{X}$的信息，数据处理不等式即如下[[#^eq-mutual-information|互信息公式]]不等式
$$
...
事实上，按照条件独立性，可知$I(\pmb{X};\pmb{Z}|\pmb{Y})=0$，又由互信息的链式关系$I(\pmb{X};\pmb{Y},\pmb{Z})=I(\pmb{X};\pmb{Y})+I(\pmb{X};\pmb{Z}|\pmb{Y})=I(\pmb{X};\pmb{Z})+I(\pmb{X};\pmb{Y}|\pmb{Z})$可知$I(\pmb{X};\pmb{Y})=I(\pmb{X};\pmb{Z})+I(\pmb{X};\pmb{Y}|\pmb{Z})$，由$I(\pmb{X};\pmb{Y}|\pmb{Z})\geq0$可证明上述不等式。
...
\tag{13}
$$
^eq-wasserstein-metric
其中$\Gamma(P,Q)$表示边缘分布分别为$P,Q$的$(\pmb{x},\pmb{y})$的联合分布的集合
$$
\begin{aligned}
\int_{\pmb{y}}\gamma(\pmb{x},\pmb{y})\mathrm{d}\pmb{y}
  &=P(\pmb{x}), \\
\int_{\pmb{x}}\gamma(\pmb{x},\pmb{y})\mathrm{d}\pmb{x}
  &=Q(\pmb{y}), \qquad \forall\gamma\in\Gamma(P,Q).
\end{aligned}
$$
...
SW_p(P,Q)
  &=\left[
      \int_{\pmb{\alpha}\in\mathrm{S}^{d-1}}
      W_p^p\big(\mathscr{R}P(\pmb{\alpha},\cdot),
      \mathscr{R}Q(\pmb{\alpha},\cdot)\big)\,\mathrm{d}S
     \right]^{\frac1p}, \\
\mathrm{d}S&=|\mathrm{d}\pmb{S}|.
\end{aligned}
\tag{17}
$$
^eq-sw-distance
...
注：[1]原点位置不随旋转发生变化，考虑$x,y$方向的单位向量$\pmb{i},\pmb{j}$及旋转后的终点状态$\tilde{\pmb{i}},\tilde{\pmb{j}}$，画垂直平分面，将得到一条经过原点的交线，即为合并旋转轴；[2]旋转变换对应$\mathbb{R}^{3\times3}$中行列式为1的正交矩阵，必有一个实数根为1，旋转轴为对应特征向量。
...
一方面，给定旋转轴及旋转角$\pmb{n}\in S^2,\theta\in\mathbb{R}$，由[[#^eq-3d-rotation-matrix|3D 旋转矩阵公式]]可得旋转所对应的线性变换$\pmb{A}$；
