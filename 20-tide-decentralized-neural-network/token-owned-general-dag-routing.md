...
因此，$\mathcal M_{\mathrm{fin}}(A)$ 是一个集合；它的每个元素

$$
\mu\in\mathcal M_{\mathrm{fin}}(A)
$$

都是一个函数 $\mu:A\to\mathbb N$。本文把这种具有有限支撑的函数本身定义为 $A$ 上的一个有限多重集，不再引入另一类对象。因而，$\mathcal M_{\mathrm{fin}}(A)$ 是许多重数函数组成的集合，而每个 $\mu\in\mathcal M_{\mathrm{fin}}(A)$ 是其中一个重数函数，也就是一个有限多重集。

对每个 $a\in A$，自然数 $\mu(a)$ 称为 $a$ 在多重集 $\mu$ 中的**重数**，表示 $a$ 出现的次数。定义中的有限性条件表示：只有有限多个 $a\in A$ 满足 $\mu(a)>0$。

例如，设 $A=\{a,b,c\}$，并定义函数 $\mu:A\to\mathbb N$ 为：
...
则 $\mu\in\mathcal M_{\mathrm{fin}}(A)$。这个函数表示“$a$ 出现两次、$b$ 不出现、$c$ 出现一次”的有限多重集。这里，$\mu$ 是函数，也是本文按上述方式定义的多重集；$\mu(a)=2$ 则是一个自然数，表示元素 $a$ 的重数。$\mu$ 与 $\mu(a)$ 不是同一类对象。

多重集用重数保存同一元素的重复出现。若两个消息具有相同载荷，但仍应被视为两个不同的消息元素，则集合 $A$ 的元素定义必须包含消息实例标识符；不能只把载荷本身作为 $A$ 的元素。
