...
第一层是一般 correctness gate：把 decode fold 展开成 time-expanded causal DAG，确认 chunk implementation 计算的是同一个 DAG、同一组 kernel equation、同一个 output/final-state extraction。这个 gate 只证明 `C_L = Fold_T^L`，不自动给出高性能。
...
数学文档中应把它们收束为两个命名 proof gate：

- `B0 Causal Chunk Theorem`：若 chunk implementation 保持同一个 time-expanded causal DAG，则 correctness 成立。
