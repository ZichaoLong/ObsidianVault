...
- 正向候选 [[token-owned-general-dag-routing]] 已把 leveled-only 约束放宽为一般 finite unit-delay DAG，并证明：若每个 node 对 owner-labelled timestamped event stream 提供 exact chunk transducer，则 absolute-time streaming schedule 与 node-topological chunk schedule 等价。该结论是 correctness theorem，不是任意 node kernel 的高性能定理。
...
| 研究 | [[token-owned-general-dag-routing]] | 一般 unit-delay DAG、timestamped token-owned event、两种同刻语义、稀疏 routing 与 node-topological chunk 证明 |
...
9. 对一般 DAG 明确选择 absolute-time event order 与 token-prefix order 的关系；分别研究同刻 owner-ordered 与 atomic-joint semantics，并把 output readout 的 autoregressive causality 写入 contract。
...
- 正向设计已经给出一般 unit-delay 空间 DAG 的 schedule-equivalence theorem，并明确区分 owner-ordered 与 atomic-joint 同刻语义。
