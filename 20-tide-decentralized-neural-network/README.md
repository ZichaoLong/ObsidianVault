...
[[semantics-anchor|语义锚点与仓库分工]] 当前固定语义版本 `tide-core-3`、共同边界、教材关系及下游引用规则。详细定义与证明由教材承担，备忘不反向改变教材。
...
当前共同接口保留聚合输入的端口、边或终端节点身份，并允许具体函数选择忽略这些标签。它还区分候选新状态、本次计算快照与下一持久状态，以及区域激活集合、各节点局部控制量与区域选择历史。状态可按统一逻辑时间衰减，或依激活结果清理。下一状态不读取完整计算结果；没有输入的时刻不自主激活或发送。时间衰减可由“保存值＋时间戳”在下一次读取时解码。
