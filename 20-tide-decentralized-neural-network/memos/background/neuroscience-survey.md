...
本页保留感觉、思考、记忆、情绪与运动的调查实质及原始文献，不承担 Tide 数学定义。调查原文见 Git `133d638` 的 `tide-background-history-and-references.md` 第二部分；这里保留其 2026-07-22 的证据范围，没有把旧调查当作本次新增的实验核验。数字模型的设计联想另见 [脑科学启发与边界](neuroscience-model-ideas.md)。

调查围绕三个问题展开：感觉信号怎样经过皮层各层和不同脑区，内部思考怎样调用记忆与控制网络，以及这些过程怎样进入动作并接收反馈。先介绍通信与解剖概念，再分别看感觉、认知和运动通路，最后区分较稳定的认识与仍有争议的解释。
...
1. **“大脑皮层总共六层”只对典型新皮层大体成立。** 人类大部分大脑半球表面的新皮层通常分为 I 到 VI 层；但海马和梨状嗅皮层所属的异皮层（allocortex），以及小脑皮层，并不是典型六层新皮层。运动皮层的 IV 层很不明显，初级视觉皮层的 IV 层则异常发达。
...
3. 当轴突起始段附近达到放电条件，神经元通常在该处产生动作电位。
...
按主要驱动输入的来源，常区分两类丘脑核团：

- **第一阶核团**：主要接收外周或皮层下驱动输入，例如视觉外侧膝状体、听觉内侧膝状体、躯体感觉腹后核。
- **高阶核团**：大量驱动输入来自皮层 V 层，再投向其他皮层区，例如枕核和部分背内侧丘脑通路。

另一条相关但不同的分类轴是**调节性皮层反馈**：皮层 VI 层广泛返回丘脑，改变丘脑增益、时序和状态。它是一类连接关系，并非与第一阶、高阶并列的第三类核团。
...
图中每个双向箭头实际上代表许多并行纤维和中间站。内部目标可以直接调动已有网络，并不需要重新经过感受器或脊髓。感觉通路也各有旁路：脊髓反射、脑干定向反应、上丘眼动等可以在完整皮层识别前启动；嗅觉不遵循“必须先经过丘脑再到初级皮层”的模板。
...
1. 脊髓局部回路可在信号到达大脑前激活同侧屈肌撤退，并抑制同侧的拮抗伸肌。在需要维持站立的情形，还可伴随对侧伸肌激活的交叉伸展反射；不能把对侧肌群笼统写成受到抑制。
...
海马的主体属于异皮层中的原皮层（archicortex），其主细胞层组织与六层新皮层不同。海马结构与周围过渡皮层也应区分；它通过内嗅皮层、海马旁皮层和广泛联合皮层交换信息。
...
- 内嗅皮层→齿状回→CA3→CA1 的三个主要突触传递阶段，常称三突触通路；CA1→下托/内嗅皮层是其后的输出与返回路线。
...
- 浦肯野细胞构成小脑皮层的输出，主要抑制深部小脑核；部分也直接投向前庭核。
...
以“阅读屏幕上的代码，找出错误并修改”为例，可以按前面介绍的一般机制作如下任务分解。这不是对某一次编程活动逐阶段脑成像的实测报告；程序理解中语言网络与多需求网络的具体贡献，不能仅从日常任务名称决定。
...
25. Legrain V, Iannetti GD, Plaghki L, Mouraux A. **The Pain Matrix Reloaded: A Salience Detection System for the Body.** Progress in Neurobiology, 2011. [DOI](https://doi.org/10.1016/j.pneurobio.2010.10.005)
...
这份调查支持把局部回路、长程通信、状态调制和身体反馈分别研究。它并未规定一个数字模型应采用多少节点、怎样做 Top-K、何时更新记忆或如何批量执行。若要从这些生物机制提出 Tide 的候选构型，下一步见 [脑科学启发与边界](neuroscience-model-ideas.md)：先明确类比落在哪个机制，再把它写成可检验的问题。
