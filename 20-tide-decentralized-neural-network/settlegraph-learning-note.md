...
本文的 SettleGraph 是一种受限的有状态图：固定节点图与区域依赖图都无环；每个输入位置上，每条边只确定一次结果，每个节点至多做一次完整计算。聚合输入保留入口、父边或终端节点标签，具体聚合可以使用这些身份，也可以只读数值。本文从一次输入的结算出发，逐步区分输入、记忆、共同选择和完整输出，再把一次次结算连接成有状态序列。符号与 [[timed-dag-region-selector-learning-note|TimedDAG 教材]] 保持一致，第 10 节给出两种对象之间的明确编码。
...
标签使数值相等的两项仍可区分，固定顺序则给它们一个规范排列。后文的聚合接收这种带身份序列：函数可以读取标签，也可以选择忽略标签而只计算值。排序是编码的一部分，不会删除身份。
...
给 $V,A$ 分别固定全序，也就是为任意两个不同元素规定相容的先后顺序，用于确定带身份聚合序列的规范排列及选择平票结果。

这里的节点和边身份是固定集合中的数学元素，不是某段字符串的数值。若要把一份规格整体重命名，必须同时搬运图、全序、局部函数以及按节点或边索引的参数；完成这种共同重命名后才是同一个结构的另一种编码。只改变标签却不搬运相应函数或参数，会得到另一份规格。
...
入口标签集合只有一个元素；非入口标签集合继承固定边序。入口节点的带身份输入序列只有图输入这一项；其他节点收集有值的父边，保留边身份并按固定边序排列：
...
这里 $i_k\in I_v$ 是入口或父边身份，$m_k\in P$ 是数值。一般聚合可以同时读取二者；相同的值序列若带有不同身份，结果可以不同。式 (5) 是忽略身份的特殊实例。更一般地，任意旧式裸值聚合 $F_v:\mathbb N\times\operatorname{Seq}^{+}(P)\to P$ 都可保守地提升为
...
因此允许读取身份不会排除任何只读值的实例。SettleGraph 的带身份序列怎样对应 TimedDAG 的原子集合，将在第 10.2 节说明。
...
这里 $\exp$ 是自然指数函数。这样，相同数值来自不同父边时可以获得不同权重。输出聚合也可把 $i_k$ 换成终端节点 $v_k$ 使用同一构造。

例如，把第 1 节的两条父边分别记为 $e_a:a\to c$ 与 $e_b:b\to c$。若它们的结果分别为 $\bot,5$，则 $\mathcal M_{c,t}=((e_b,5))$，式 (5) 的均值为 $5$。无值边不以零填充，也不占均值分母；但 $e_b$ 的身份仍在输入中，其他聚合可以使用它。因为固定标签集合 $I_c$ 也是规格的一部分，聚合还可从实际标签集合看出 $e_a$ 本次缺席；它仍没有收到一个名为 $\bot$ 的数值项。
...
1. 已确定父边结果后，由式 (3)--(5) 得到带身份输入序列、候选集合与本地内容。
...
下文证明这个序列非空，所以式 (15) 不会越出定义域。输出聚合可以读取终端节点身份。任意旧式裸值输出聚合 $F_{\mathrm{out}}:\mathbb N\times\operatorname{Seq}^{+}(P)\to P$ 都可提升为

$$
\operatorname{Agg}_{\mathrm{out}}
(\theta,((v_1,m_1),\ldots,(v_n,m_n)))
=F_{\mathrm{out}}(\theta,(m_1,\ldots,m_n)),
$$

所以忽略第一坐标就恢复只读取终端值的特殊实例。
...
1. 每个 $t\in[L]$、$v\in V$ 的带身份输入序列 $\mathcal M_{v,t}$，以及每条 $a\in A$ 的边结果 $z_{a,t}$；
...
6. 每个 $t\in[L]$ 的带终端身份值序列 $\mathcal M_{\mathrm{out},t}$ 与最终输出 $b_t$。
...
在第一个位置，$z_{e_a,0}=\bot$，$z_{e_b,0}=5$。所以 $c$ 的聚合输入是单项序列 $((e_b,5))$，不是补零后的 $((e_a,0),(e_b,5))$，其均值为 $5$。第二个位置两条边的角色相反。虽然本例均值忽略 $e_b$，带身份输入仍允许另一个聚合对 $e_a,e_b$ 采用不同参数。
...
输出节点的聚合根据入边 $a_v^{\mathrm{out}}$ 确定原终端节点 $v$，按原节点序排列 $(v,m_v)$，再把这个带身份序列交给 $\operatorname{Agg}_{\mathrm{out}}$，时间参数仍为 $\theta$。这个定义在非空、每条入边至多一个原子且所有原子的到达时间均为 $\theta$ 时使用；其他自变量统一返回零向量，使聚合在 TimedDAG 要求的整个定义域上为全函数。输出节点的 Full 在唯一端口 $o_\star$ 返回 $h$，内部出边族为空。

两个新节点只是编码中的边界适配对象，不能算作原 SettleGraph 新增的可训练层。入口、终端宽度仍属于需要核算的边界成本。

### 10.2 消息为何对齐，聚合身份如何保持
...
原节点的聚合按以下规则定义为 TimedDAG 的全函数。对入口节点的非空纤维，把唯一新入边消息映成标签 $\mathrm{in}_v$ 与其数值组成的单项序列；对非入口节点，保留每个消息原子的原边 $a$ 与数值，按原入边序排列，恰得到式 (3) 的带身份序列。随后应用原来的 $\operatorname{Agg}_v$。在空集合、同一边多次出现、原子到达时间不全等于 $\theta$ 等其他自变量上，统一返回零向量。实际计算不会进入这些额外情形；此举只把函数完整定义在 TimedDAG 的原子集合输入类型上。

不同父边即使携带相同数值，仍是不同原子。上述映射同时保留边身份、规范顺序和数值，因此 SettleGraph 聚合读取身份时也得到与直接语义相同的自变量。只读值的聚合则是同一接口中忽略标签的函数选择。
...
由完整消息记录与固定槽位，可以反向恢复每个 $z_{a,t}$，再按式 (3) 恢复全部带身份序列。适配节点和它们的边是辅助坐标；删除这些坐标并作上述还原，就得到第 6.3 节的完整记录 $\mathcal T_x^{\mathrm S}$。

> [!theorem] 定理 4：向 TimedDAG 的记录嵌入
> 上述编码在全部原节点、原区域和输入位置满足上述对应关系；包括本地量、选择、控制、计算快照、持久状态、历史、全部边结果和输出。

**证明。** 对输入位置归纳，并在同一位置内按区域次序归纳。位置 $0$ 的原节点和区域初态按构造相同。输入适配节点把同一值送至各入口。式 (18) 使全部原父消息与目标位置对齐；前驱归纳保证边身份与携带值相同。聚合标签、顺序和数值均一致，故带身份输入序列与本地内容相同。状态、读取、选择、快照、延续与完整输出函数相同，因而本区域全部相应坐标相同；非候选节点和空候选区域则分别保持状态与历史。输出适配节点把每条新入边恢复为同一终端节点身份，收集同一带身份序列并应用同一聚合，输出相同。
...
身份标签在纤维关闭时已经随消息确定，因此身份感知本身不会增加 Full 与控制之间的依赖，也不改变上述外层批次数。这里仍按分块教材的默认成本 profile 把 Agg 归入控制作用。若具体 Agg 对每条边应用大矩阵等昂贵运算，就必须把这部分加入昂贵作用范围，并为它另给精确 batch witness；不能仅因接口名称仍是 Agg 就把主要工作记作廉价控制。
