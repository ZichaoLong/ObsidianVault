...
给 $V,A$ 分别固定全序，也就是为任意两个不同元素规定相容的先后顺序，用于确定聚合顺序及选择平票结果。
...
这里 $m_k\in P$ 是数值。聚合读取值的顺序和重数，但不额外读取父边身份；相同的值序列必得到相同的聚合结果。它与 TimedDAG 的一般聚合有何区别，将在第 10.2 节说明。

例如，第 1 节的两条父边若分别为 $\bot,5$，则 $\mathcal M_{c,t}=(5)$，均值为 $5$。无值边不以零填充，也不占均值分母。
